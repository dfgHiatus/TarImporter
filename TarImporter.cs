...
using TarImporter.Extractor;

namespace TarImporter;

public class TarImporter : ResoniteMod
{
    ...
    private readonly static string CachePath = Path.Combine(Engine.Current.CachePath, "Cache", "DecompressedTars");
    ...
    {
        new Harmony("net.dfgHiatus.TarImporter").PatchAll();
        Config = GetConfiguration();
        Directory.CreateDirectory(CachePath);
    }
    
    public static string[] DecomposeTars(string[] files)
    {
        ...
        foreach (var package in targzsToDecompress)
        {
            ...
            TarExtractor.Unpack(package, extractedPath);
    ...
    public class UniversalImporterPatch
    {
        static bool Prefix(ref IEnumerable<string> files)
        {
            ...
            {
                if (Path.GetExtension(file).ToLower() == TAR_FILE_EXTENSION)
                    hasTarGZ.Add(file);
                else
                    notTarGZ.Add(file);
            }
            ...
            foreach (var dir in DecomposeTars(hasTarGZ.ToArray()))
                allDirectoriesToBatchImport.AddRange(Directory.GetFiles(dir, "*", SearchOption.AllDirectories)
                    .Where(ShouldImportFile).ToArray());

            var slot = Engine.Current.WorldManager.FocusedWorld.AddSlot("Tar import");
    ...
    private static bool ShouldImportFile(string file)
    {
        ...
        return (Config.GetValue(importText) && assetClass == AssetClass.Text) 
            ...
            || extension == TAR_FILE_EXTENSION;                                    // Handle recursive tar imports
    }
